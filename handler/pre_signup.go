...
func PreSignupHandler(
	...
	/*
		...
		認証方法が違うと、メールアドレスが同じでも異なるユーザとして扱われてしまうため、重複するユーザが存在する場合エラーにする
		SOLID にならないが、migrate_user コントローラー（移行トリガー発火時の実装）で firebase に存在するかチェックしているためここではチェックしない
		（※ 米国のデータストアに毎回確認しにいく firebase 処理を行う回数を減らすことを優先）

		また、現状（2024/10/09）機能開発のなかでそれが使用される可能性は低いので後者も無視
	*/
